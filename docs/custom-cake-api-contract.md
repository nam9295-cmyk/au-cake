...
## 11. Authenticated administrator request list

`admin-list-custom-cake-requests` is an existing-admin-authenticated `custom-cake.v1` action with no `data` member. Anonymous or non-admin requests fail with `FORBIDDEN`. Success is `{ok:true,result:{requests:CustomCakeLookupResponse[]}}`; the response contains only the strict lookup snapshot wire and never returns persistence envelopes, raw `payloadJson`, transition audit, commit records, credentials, tokens or internal metadata. A malformed stored snapshot fails closed rather than being partially projected.

The server reads snapshot pages in ascending document-ID order using 100 records per page, scans at most 10 pages (1,000 stored snapshots), then sorts valid Custom Cake requests by immutable `quote.promotionEligibilityAt` descending with request number as the deterministic tie-breaker. The response is bounded to the newest 200 requests. This endpoint does not add a new customer lookup or relax any mutation trust boundary; quote/lifecycle/photo mutations continue to require the independently verified administrator JWT.
