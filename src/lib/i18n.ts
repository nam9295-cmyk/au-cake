...
export function cakeCopy(language: Language) {
  if (language === 'ko') {
    return {
      ...
      pickupDate: '픽업 날짜',
      pickupTime: '픽업 시간',
      pickupLeadTimeHelp: '오후 8시 전에 주문하면 다음 날 오픈 시간부터, 오후 8시 이후에는 다음 날 낮 12시부터 픽업할 수 있어요.',
  ...
  return {
    ...
    announcement: 'Order before 8pm for next-day pick-up · From 8pm, pick up tomorrow from 12pm',
    ...
    pickupLeadTimeHelp: 'Order before 8pm for pick-up from opening time tomorrow. Orders from 8pm can be picked up tomorrow from 12pm.',
