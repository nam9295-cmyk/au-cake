...
import { formatCents, formatExtraCents, getStatusInfo } from '../lib/custom-cake-client.js'
    ...
    return (
      <>
        ...
        <main className="narrow-page completion-page">
          ...
          <p>{language === 'ko' ? '새로운 커스텀 케이크 요청을 작성해 주세요.' : 'Please start a new custom cake request.'}</p>
          <button className="primary-button" type="button" onClick={() => navigate('cakes')}>
            {language === 'ko' ? '케이크 목록 보기' : 'Browse Cakes'}
          </button>
        </main>
      </>
    )
          ...
          <section className="quote-summary-section">
            ...
            <dl className="quote-summary-table">
              ...
              <div className="summary-row total">
                <dt>
                  <strong>{language === 'ko' ? '잠정 확인 금액' : 'Estimated Known Total'}</strong>
                </dt>
                <dd>
