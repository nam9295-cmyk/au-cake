...
사진 정규화 dependency인 `sharp@0.35.4`는 Node 20.9 이상이 필요합니다. Review API recovery deploy target은 exact `node-22`이며 `node-16.0`, `node-20.0`, `node-22.0` 같은 다른 runtime ID는 fail-closed로 거부합니다. Appwrite runtime registry와 `_APP_FUNCTIONS_RUNTIMES` allowlist에 exact `node-22`가 준비되지 않았다면 배포를 강행하지 마세요. 현재 healthy `node-20.0` deployment는 Node 22 rollout smoke test가 끝날 때까지 rollback artifact로 유지합니다.
