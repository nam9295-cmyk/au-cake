...
test('storage absence, incomplete chunks, public metadata or failed reads never prove settled upload', async () => {
  const m = await load(); assert.equal(typeof m.createCustomCakePhotoStorage, 'function')
  const good = { $id: 'f1', $permissions: [], mimeType: 'image/webp', chunksTotal: 1, chunksUploaded: 1, sizeOriginal: 2 }
  for (const record of [null, { ...good, chunksUploaded: 0 }, { ...good, $permissions: ['read("any")'] }, { ...good, sizeOriginal: 10485761 }, { ...good, mimeType: 'image/jpeg' }, good]) {
    const sdk = { async createFile() {}, async deleteFile() {}, async getFile() { if (!record) throw Object.assign(new Error('missing'), { code: 404 }); return record }, async getFileDownload() { throw new Error('uncertain') } }
    const storage = m.createCustomCakePhotoStorage(sdk, { bucketId: 'private', privateBucketVerified: true })
