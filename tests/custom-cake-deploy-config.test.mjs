...
test('Node22 deployment requires explicit custom opt-in, exact runtime and recovery schedule', () => {
  for (const config of [api, notification]) {
    ...
    assert.equal(result.runtime, 'node-22.0')
    ...
    assert.throws(() => config.resolveDeployConfig({ ...enabled, APPWRITE_RESERVATION_API_RUNTIME: 'node-16.0', APPWRITE_RESERVATION_NOTIFY_RUNTIME: 'node-16.0' }), /node-22.0/)
  ...
  assert.deepEqual(api.buildRuntimeCandidates('node-22.0'), ['node-22.0'])
})
