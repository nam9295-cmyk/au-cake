...
test('new catalogue photos and previous detail photos use descriptive canonical files', async () => {
  ...
  assert.match(catalogSource, /id: 'smore-stick',[\s\S]*?defaultProductId: 'smore-stick'/)
...
test('AU homepage catalogue keeps its matched desktop and mobile card-to-cake proportions', async () => {
  ...
  assert.match(css, /@media \(min-width: 768px\)[\s\S]*?\.product-section \.cake-catalog-group-products \.product-card-detail-link,\s*\.product-section \.cake-catalog-group-products \.product-card-price\s*\{[^}]*width:\s*min\(92%, clamp\(288px, 30vw, 420px\)\)[^}]*margin-inline:\s*auto/s)
