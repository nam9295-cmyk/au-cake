...
test('SEO generator writes shared homepage content, cake pages, and AU sitemap', async () => {
  ...
  assert.match(home, /Pave, fresh cream, buttercream, cupcakes, gâteau au chocolat, lemon cake and brownie cheesecake from AUD 31/)
...
test('cake generator uses the final per-page schema contract and real product WebPs', async () => {
  ...
  const expectations = new Map([
    ...
    ['brownie-cheesecake', { type: 'Product', price: 55, og: 'product', image: 'brownie-cheese-sydney.webp' }],
  ])
...
test('llms text exposes only grounded public catalogue and ordering facts', async () => {
  ...
  assert.match(llms, /Chocolate cake sheets layered with plain fresh cream/)
  assert.match(llms, /Chocolate Buttercream between chocolate cake sheets/)
