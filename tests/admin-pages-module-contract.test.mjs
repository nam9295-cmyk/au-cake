...
test('App delegates the named admin pages and drawers to explicit top-level modules', () => {
  assert.match(appSource, /import\('\.\/AdminDashboardPage'\)/)
  assert.match(appSource, /import\('\.\/AdminLoginPage'\)/)
  assert.match(appSource, /import\('\.\/AdminReservationsPage'\)/)
  assert.match(appSource, /import\('\.\/AdminClassesPage'\)/)
