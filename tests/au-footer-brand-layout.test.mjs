...
test('public routes mount a responsive branded footer with the supplied cutout assets', () => {
  ...
  assert.match(footer, /Chocolate-first cakes made with chocolatier-grade couverture chocolate and finished by hand in Melrose Park\./)
  assert.match(footer, /쇼콜라티에용 커버춰 초콜릿으로 만들고, 멜로즈 파크에서 손으로 정성껏 완성합니다\./)
